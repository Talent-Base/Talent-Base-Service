...
class VagaDeEmpregoRepository:
    ...
    def getVagaDeEmpregoByEmpresaId(id_empresa: int, database: Session = Depends(getDatabase)):
        vaga_de_emprego = database.query(VagaDeEmprego).filter(id_empresa == id_empresa).all()
        return vaga_de_emprego
