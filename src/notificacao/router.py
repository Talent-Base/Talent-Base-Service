...
@router.post("/")
async def createNotificacao(
    new_notificacao: NotificacaoBase, database: Session = Depends(getDatabase)
):
    new_notificacao = NotificacaoRepository.createNotificacao(
        Notificacao(**new_notificacao.model_dump()), database
    )
