...
def requireAdminGestor(user: Usuario = Depends(getCurrentActiveUser)):
	if user.papel != 'admin' and user.papel != 'gestor':
		raise HTTPException(
			status_code=status.HTTP_403_FORBIDDEN,
			detail='Candidatos não podem acessar esta rota.',
		)
	return user


def requireAdminCandidato(user: Usuario = Depends(getCurrentActiveUser)):
	if user.papel != 'admin' or user.papel != 'candidato':
		raise HTTPException(
			status_code=status.HTTP_403_FORBIDDEN,
			detail='Candidatos não podem acessar esta rota.',
		)
