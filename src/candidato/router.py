...
async def getCandidatosExperiencias(id_candidato: int, database: Session = Depends(getDatabase)):
    experiencias = ExperienciaRepository.getExperienciasByCandidatoId(id_candidato, database)
    if not experiencias:
        raise HTTPException(status_code=404, detail="Experiências não encontradas")
    return experiencias

@router.post("/{id_candidato}/experiencias")
async def createCandidatosExperiencia(id_candidato: int, experiencia_data: ExperienciaBase, database: Session = Depends(getDatabase)):
    new_experiencia = ExperienciaRepository.createExperiencia(Experiencia(**experiencia_data.model_dump()), database)
    return new_experiencia

@router.put("/{id_candidato}/experiencias/{id_experiencia}")
async def updateCandidatosExperienciaById(id_candidato: int, id_experiencia: int, database: Session = Depends(getDatabase)):
...
async def deleteCandidatosExperiencia(id_candidato: int, id_experiencia: int, database: Session = Depends(getDatabase)):
    experiencia = ExperienciaRepository.getExperienciaById(id_experiencia, database)
    if not experiencia:
...
async def getCandidatosCandidaturas(id_candidato: int, database: Session = Depends(getDatabase)):
    candidaturas = CandidaturaRepository.getCandidaturasByCandidatoId(id_candidato, database)
    if not candidaturas:
        raise HTTPException(status_code=404, detail="Candidaturas não encontradas")
    return candidaturas

@router.post("/{id_candidato}/candidaturas")
async def createCanditadosCandidatura(id_candidato: int, candidatura_data: CandidaturaBase, database: Session = Depends(getDatabase)):
    candidatura_already_exists = CandidaturaRepository.candidaturaExists(id_candidato, candidatura_data.id_vaga_de_emprego, database)
    if candidatura_already_exists:
        raise HTTPException(status_code=404, detail="Candidatura já realizada")
    candidatura = CandidaturaRepository.createCandidatura(Candidatura(**candidatura_data.model_dump()), database)
    return candidatura

@router.delete("/{id_candidato}/candidaturas/{id_candidatura}")
async def deleteCandidatoCandidatura(id_candidato: int, id_candidatura: int, database: Session = Depends(getDatabase)):
...
async def getCandidatosNotificacoes(id_candidato: int, database: Session = Depends(getDatabase)):
    notificacoes = NotificacaoRepository.getNotificacoesByCandidatoId(id_candidato, database)
    if not notificacoes:
...
async def updateNotificacaoById(id_candidato: int, id_notificacao: int, notificacao_data: NotificacaoBase, database: Session = Depends(getDatabase)):
