...
def test_createExperiencia(add_candidato, experiencia, db):
    ...
    experiencias_db = (
        db.query(Experiencia).filter_by(id_usuario=add_candidato.id_usuario).all()
    )
...
@pytest.mark.parametrize(
    "nova_instituicao", ["Instituição 1", "Instituição 2", "Instituição 3"]
)
