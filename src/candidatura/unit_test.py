...
from ..models import Candidatura, Usuario, VagaDeEmprego, Candidato, Empresa
...
def add_usuario(db):
	...
	return UsuarioRepository.createUsuario(usuario, db)


@pytest.fixture
def add_candidato(add_usuario, db):
	sample = Candidato(
		id_candidato=add_usuario.id,
		nome='João Silva',
		email='joao@example.com',
		estado='SP',
		cidade='São Paulo',
...
def test_createCandidatura(candidatura, add_candidato, add_vaga_de_emprego, db):
	candidatura.id_candidato = add_candidato.id_candidato
...
def test_candidaturaExists(candidatura, add_candidato, add_vaga_de_emprego, db):
	...
	result = CandidaturaRepository.candidaturaExists(
		candidatura.id_candidato, candidatura.id_vaga_de_emprego, db
	)
	assert result
...
@pytest.mark.parametrize('candidatura', make_candidaturas())
def test_nonExistentCandidatura_candidaturaExists(
	candidatura, add_candidato, add_vaga_de_emprego, db
):
	result = CandidaturaRepository.candidaturaExists(
		candidatura.id_candidato, candidatura.id_vaga_de_emprego, db
	)
...
def test_getCandidaturaById(candidatura, add_candidato, add_vaga_de_emprego, db):
	...
	result = CandidaturaRepository.getCandidaturaById(
		created_candidatura.id_candidatura, db
	)
	assert result.data == candidatura.data


def test_nonExistentCandidatura_getCandidaturaById(db):
	result = CandidaturaRepository.getCandidaturaById(-1, db)
	assert result is None


@pytest.mark.parametrize('novo_status', ['Rejeitado', 'Aprovado'])
def test_updateCandidatura(
	sample_candidatura, novo_status, add_candidato, add_empresa, db
):
...
@pytest.mark.parametrize('candidatura', make_candidaturas())
def test_nonExistentCandidatura_deleteCandidatura(
	candidatura, add_candidato, add_vaga_de_emprego, db
):
