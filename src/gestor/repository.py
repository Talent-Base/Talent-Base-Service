...
class GestorRepository:
    ...
    def getGestoresByEmpresaId(id_empresa: int, database: Session = Depends(getDatabase)):
        gestores = database.query(Gestor).filter(Gestor.id_empresa == id_empresa).all()
        return gestores
        
    def gestorExistsByEmail(email_gestor: int, database: Session = Depends(getDatabase)):
