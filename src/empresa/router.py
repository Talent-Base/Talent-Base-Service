...
async def createEmpresa(empresa_data: EmpresaBase, database: Session = Depends(getDatabase)):
    empresa_already_exists = EmpresaRepository.empresaExistsByCnpj(empresa_data.cnpj, database)
    if empresa_already_exists:
        ...
        new_empresa = EmpresaRepository.createEmpresa(Empresa(**empresa_data.model_dump()), database)
        return new_empresa
...
async def getEmpresasVagaDeEmprego(id_empresa: int, database: Session = Depends(getDatabase)):
    vagas_de_emprego = VagaDeEmpregoRepository.getVagaDeEmpregoByEmpresaId(id_empresa, database)
...
async def getEmpresasGestores(id_empresa: int, database: Session = Depends(getDatabase)):
